...
function toggleEditRole(code: string) {
  if (editForm.roleCodes.includes(code)) {
    editForm.roleCodes = editForm.roleCodes.filter((item) => item !== code)
  } else {
    editForm.roleCodes = [...editForm.roleCodes, code]
  }
}
...
function startEdit(user: UserProfile) {
  ...
  editForm.classId = user.classId || 0
...
async function submitEdit(user: UserProfile) {
  if (editForm.roleCodes.length === 0) {
    error.value = '至少选择一个角色'
    return
  }
  saving.value = true
  error.value = ''
  ...
  try {
    ...
    message.value = `${user.username} 已更新`
    cancelEdit()
    await load()
  } catch (e) {
    error.value = e instanceof Error ? e.message : '用户更新失败'
  } finally {
    saving.value = false
  }
}
        ...
        <table class="min-w-full divide-y divide-slate-200 dark:divide-white/10">
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-white/10">
            <template v-for="user in users" :key="user.id">
              ...
              <tr v-if="editingUserId === user.id" class="bg-slate-50/80 dark:bg-white/5">
                <td colspan="6" class="px-5 py-4">
                  <form class="grid gap-4 md:grid-cols-3" @submit.prevent="submitEdit(user)">
                    <label>
                      <span class="text-sm font-medium text-slate-700 dark:text-slate-200">真实姓名</span>
                      <input v-model.trim="editForm.realName" class="focus-ring mt-2 block w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-950 dark:border-white/10 dark:bg-slate-900 dark:text-white" />
                    </label>
                    <label>
                      <span class="text-sm font-medium text-slate-700 dark:text-slate-200">邮箱</span>
                      <input v-model.trim="editForm.email" type="email" class="focus-ring mt-2 block w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-950 dark:border-white/10 dark:bg-slate-900 dark:text-white" />
                    </label>
                    <label>
                      <span class="text-sm font-medium text-slate-700 dark:text-slate-200">手机号</span>
                      <input v-model.trim="editForm.phone" class="focus-ring mt-2 block w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-950 dark:border-white/10 dark:bg-slate-900 dark:text-white" />
                    </label>
                    <label>
                      <span class="text-sm font-medium text-slate-700 dark:text-slate-200">性别</span>
                      <select v-model.number="editForm.gender" class="focus-ring mt-2 block w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-950 dark:border-white/10 dark:bg-slate-900 dark:text-white">
                    ...
                    <label>
                      ...
                      <select v-model.number="editForm.status" class="focus-ring mt-2 block w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-950 dark:border-white/10 dark:bg-slate-900 dark:text-white">
                        <option :value="1">正常</option>
                        <option :value="0">禁用</option>
                        <option :value="2">锁定</option>
                      </select>
