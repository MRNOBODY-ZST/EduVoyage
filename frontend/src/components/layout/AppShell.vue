...
<template>
  <div class="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-white">
              ...
              <div class="flex grow flex-col gap-y-5 overflow-y-auto border-r border-slate-200 bg-white px-6 pb-4 dark:border-white/10 dark:bg-slate-900">
                ...
                <nav class="flex flex-1 flex-col">
                  <ul role="list" class="flex flex-1 flex-col gap-y-7">
                      ...
                      <ul role="list" class="-mx-2 space-y-1">
                        <li v-for="item in visibleNavigation" :key="item.to">
                          <RouterLink
                            ...
                            :class="[
                              isActive(item)
                                ? 'bg-[rgb(var(--color-brand-soft))] text-[rgb(var(--color-brand))]'
                              ...
                              'group flex gap-x-3 rounded-md p-2 text-sm/6 font-semibold dark:bg-transparent',
                            ]"
                          ...
                          >
                            <component
                              :is="item.icon"
                              :class="[
                                isActive(item) ? 'text-[rgb(var(--color-brand))]' : 'text-slate-400 group-hover:text-[rgb(var(--color-brand))]',
                                'size-6 shrink-0',
                              ]"
                              aria-hidden="true"
                            />
                    ...
                    <li>
                      <div class="text-xs/6 font-semibold text-slate-400 dark:text-slate-500">{{ t('nav.studentGroups') }}</div>
                            ...
                            <span
                              :class="[
                                isActive(team)
                                  ...
                                  : 'border-slate-200 text-slate-400 group-hover:border-[rgb(var(--color-brand))] group-hover:text-[rgb(var(--color-brand))] dark:border-white/15 dark:text-slate-500',
                                'flex size-6 shrink-0 items-center justify-center rounded-lg border bg-white text-[0.625rem] font-medium dark:bg-transparent',
                              ]"
                            >
                              {{ team.initial }}
                            </span>
                    ...
                    <li class="mt-auto">
                      <RouterLink
                        to="/settings"
                        class="group -mx-2 flex gap-x-3 rounded-md p-2 text-sm/6 font-semibold text-slate-700 hover:bg-slate-50 hover:text-[rgb(var(--color-brand))] dark:text-slate-300 dark:hover:bg-white/5"
                        @click="sidebarOpen = false"
                      >
                        <Cog6ToothIcon class="size-6 shrink-0 text-slate-400 group-hover:text-[rgb(var(--color-brand))]" aria-hidden="true" />
    ...
    <!-- Main content -->
    <div class="lg:pl-72">
      <header class="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-4 border-b border-slate-200 bg-white px-4 shadow-xs sm:gap-x-6 sm:px-6 lg:px-8 dark:border-white/10 dark:bg-slate-900">
        <button type="button" class="-m-2.5 p-2.5 text-slate-700 lg:hidden dark:text-slate-300" :title="t('shell.openSidebar')" @click="sidebarOpen = true">
        ...
        <div class="grid flex-1 grid-cols-1">
          <input
            ...
            class="col-start-1 row-start-1 block size-full bg-transparent py-1.5 pl-8 text-base text-slate-900 outline-none placeholder:text-slate-400 sm:text-sm/6 dark:text-white dark:placeholder:text-slate-500"
        ...
        <div class="flex items-center gap-x-3 lg:gap-x-4">
          ...
          <div class="hidden lg:block lg:h-6 lg:w-px lg:bg-slate-200 dark:lg:bg-white/10" aria-hidden="true" />

          <Menu as="div" class="relative">
            <MenuButton class="-m-1.5 flex items-center p-1.5">
              <span class="sr-only">{{ t('shell.openUserMenu') }}</span>
              <span class="grid size-8 place-items-center rounded-full bg-[rgb(var(--color-brand-soft))] text-sm font-semibold text-[rgb(var(--color-brand))]">
                {{ userInitial }}
              </span>
              <span class="hidden lg:flex lg:items-center">
                <span class="ml-4 text-sm/6 font-semibold text-slate-900 dark:text-white" aria-hidden="true">{{ auth.displayName }}</span>
                <ChevronDownIcon class="ml-2 size-5 text-slate-400" aria-hidden="true" />
              </span>
            </MenuButton>
            <transition
            ...
            >
              <MenuItems class="absolute right-0 z-10 mt-2.5 w-44 origin-top-right rounded-xl bg-white py-2 shadow-lg ring-1 ring-slate-900/5 focus:outline-none dark:bg-slate-800 dark:ring-white/10">
                <MenuItem v-slot="{ active }">
                  <div :class="[active ? 'bg-slate-50 dark:bg-white/5' : '', 'px-3 py-2']">
                    <p class="truncate text-sm font-semibold text-slate-900 dark:text-white">{{ auth.displayName }}</p>
                    <p class="mt-0.5 text-xs text-slate-500 dark:text-slate-400">{{ roleLabel(auth.primaryRole) }}</p>
                  </div>
                </MenuItem>
                <MenuItem v-slot="{ active }">
                  <RouterLink :class="[active ? 'bg-slate-50 dark:bg-white/5' : '', 'block px-3 py-1.5 text-sm/6 text-slate-700 dark:text-slate-300']" to="/settings">
                    {{ t('nav.personalSettings') }}
                  </RouterLink>
                </MenuItem>
                <MenuItem v-slot="{ active }">
                  <button
                    type="button"
                    :class="[active ? 'bg-slate-50 dark:bg-white/5' : '', 'block w-full px-3 py-1.5 text-left text-sm/6 text-slate-700 dark:text-slate-300']"
                    @click="logout"
                  >
                    {{ t('shell.logout') }}
                  </button>
      ...
      <main class="py-10">
        <div class="px-4 sm:px-6 lg:px-8">
          <div class="mb-8">
            <p class="text-sm/6 font-medium text-slate-500 dark:text-slate-400">{{ t('app.name') }}</p>
            <h1 class="mt-2 text-2xl font-semibold text-slate-900 dark:text-white">{{ currentTitle }}</h1>
          </div>
          <RouterView />
        </div>
      </main>
